  ...
  return (
    <>
      <Dropdown
        ...
        onSelectId={id =>
          questionerAdminTsjrpcClient.setTemplateConditionOperator({ blankw, templateId, operator: id })
        }
      />
            ...
            <Card.Content>
              <Card.Header>
                <div className="flex justify-between gap-3 flex-wrap">
                  <Button
                    ...
                    className="text-xKO"
                    onClick={() =>
                      questionerAdminTsjrpcClient.removeTemplateCondition({ blankw, nexti: conditioni, templateId })
                    }
                  />
                  <Button
                    disabled={condition.next.length === 1}
                    onClick={() =>
                      questionerAdminTsjrpcClient.setTemplateConditionOperator({
                        blankw,
                        templateId,
                        nexti: conditioni,
                  ...
                  >
                    {template?.if?.next[conditioni].t ? 'ИЛИ' : 'И'}
                  </Button>
                </div>
              </Card.Header>
                    ...
                    <div className="flex gap-3 flex-wrap">
                      <Dropdown
                        ...
                        items={mylib.keys(blank.tmp).map(id => {
                          ...
                          return {
                            id: +id,
                            title: blank.tmp[id].title ?? questionerTemplateDescriptions[blank.tmp[id].type].title,
                          };
                        })}
                        onSelectId={conditionTemplateId =>
                          questionerAdminTsjrpcClient.setTemplateConditionNextTemplateId({
                            blankw,
                            templateId,
                            conditionTemplateId,
                            nexti: conditioni,
                      ...
                      {nextTemplate &&
                        questionerCardContents(nextTemplate.type).conditionConfigureRender({
                          ifCondition: template.if?.next[conditioni].next[nextNexti] as never,
                          operator:
                            template.if?.next[conditioni]?.next[nextNexti]?.op ??
                            questionerTypeDefaultIfConditionOperatorDict[nextTemplate.type],
                      ...
                      {condition.next.length === 1 || (
                        <Button
                          icon="Delete02"
                          className="text-xKO"
                          onClick={() =>
                            questionerAdminTsjrpcClient.removeTemplateCondition({
                              blankw,
                              nexti: conditioni,
                              templateId,
                              nextNexti,
              ...
              <Card.Footer className="flex flex-wrap gap-3 mt-5">
                <Button
                  icon="PlusSign"
                  onClick={() =>
                    questionerAdminTsjrpcClient.addTemplateConditionNextNext({
                      blankw,
                      templateId,
                      nexti: conditioni,
                    })
