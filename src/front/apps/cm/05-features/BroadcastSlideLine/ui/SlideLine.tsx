...
export const CmBroadcastSlideLine = ({ topNode }: { topNode: (nodes?: ReactNode) => ReactNode }) => {
  const { slidei: currentSlidei, setSlidei, slides, toSlide } = useCmBroadcastSlidesContext();
    ...
    <>
      ...
      <div className="no-scrollbar snap-x snap-mandatory flex my-2 bg-x1 py-2 overflow-auto nowrap rounded-md">
        {slides.map((slide, slidei) => {
          if (!slide || (showChordedSlideMode === CmBroadcastShowChordedSlideMode.Hide && slide.ord.isChBlock()))
            return;
              ...
              >
                <div
                  {...makeCmComTextInnerHtmlProp(
                    squareBracketsReplacers[CmComTextSquareBracketsMode.Remove](slide.text),
                  )}
                />
              </div>
            </div>
