import React, { FC, JSX } from 'react';
import { TheIconWrapper } from '../icon-wrapper';
import { TheIconProps, TheIconSelfPack } from '../model';

const d: Record<string, string> = {
  d1: 'M21 21H10C6.70017 21 5.05025 21 4.02513 19.9749C3 18.9497 3 17.2998 3 14V3',
  d2: 'M6 6H6.00898M8.9982 6H9.00718M11.9964 6H12.0054M14.9946 6H15.0036M17.9928 6H18.0018M20.991 6H21',
  d3: 'M19 21C19 14.3726 16.3137 9 13 9C9.68629 9 7 14.3726 7 21',
  d4: 'M5 6C5 5.44772 5.44772 5 6 5H6.00898C6.56126 5 7.00898 5.44772 7.00898 6C7.00898 6.55228 6.56126 7 6.00898 7H6C5.44772 7 5 6.55228 5 6ZM7.9982 6C7.9982 5.44772 8.44592 5 8.9982 5H9.00718C9.55947 5 10.0072 5.44772 10.0072 6C10.0072 6.55228 9.55947 7 9.00718 7H8.9982C8.44592 7 7.9982 6.55228 7.9982 6ZM10.9964 6C10.9964 5.44772 11.4441 5 11.9964 5H12.0054C12.5577 5 13.0054 5.44772 13.0054 6C13.0054 6.55228 12.5577 7 12.0054 7H11.9964C11.4441 7 10.9964 6.55228 10.9964 6ZM13.9946 6C13.9946 5.44772 14.4423 5 14.9946 5H15.0036C15.5559 5 16.0036 5.44772 16.0036 6C16.0036 6.55228 15.5559 7 15.0036 7H14.9946C14.4423 7 13.9946 6.55228 13.9946 6ZM16.9928 6C16.9928 5.44772 17.4405 5 17.9928 5H18.0018C18.5541 5 19.0018 5.44772 19.0018 6C19.0018 6.55228 18.5541 7 18.0018 7H17.9928C17.4405 7 16.9928 6.55228 16.9928 6ZM19.991 6C19.991 5.44772 20.4387 5 20.991 5H21C21.5523 5 22 5.44772 22 6C22 6.55228 21.5523 7 21 7H20.991C20.4387 7 19.991 6.55228 19.991 6Z',
  d5: 'M3 2C3.55229 2 4 2.44772 4 3V14C4 15.6782 4.00213 16.8362 4.11923 17.7072C4.23246 18.5494 4.43644 18.972 4.73223 19.2678C4.99508 19.5306 5.35803 19.721 6.02661 19.8396C6.164 16.8568 6.827 14.1394 7.86293 12.0675C9.00187 9.78963 10.7603 8 13 8C15.2397 8 16.9981 9.78963 18.1371 12.0675C19.1915 14.1765 19.8596 16.9543 19.9803 20H21C21.5523 20 22 20.4477 22 21C22 21.5523 21.5523 22 21 22H9.928C8.92293 22 8.03613 22 7.26199 21.9653C7.17849 21.9879 7.09066 22 7 22C6.86608 22 6.73831 21.9737 6.62157 21.9259C6.41367 21.909 6.21529 21.8884 6.0263 21.8629C4.96232 21.7199 4.04735 21.4113 3.31802 20.682C2.58869 19.9526 2.28011 19.0377 2.13706 17.9737C1.99994 16.9538 1.99997 15.6607 2 14.072L2 3C2 2.44772 2.44772 2 3 2ZM8.02192 19.9883C8.58939 19.9996 9.24158 20 10 20H17.9786C17.8588 17.2166 17.2449 14.7554 16.3482 12.9619C15.3156 10.8967 14.074 10 13 10C11.926 10 10.6844 10.8967 9.65179 12.9619C8.75632 14.7529 8.14291 17.2097 8.02192 19.9883Z',
  d6: 'M9.65179 12.9619C8.64906 14.9674 8 17.8079 8 21H6C6 17.5647 6.69409 14.4052 7.86293 12.0675C9.00187 9.78963 10.7603 8 13 8C15.2397 8 16.9981 9.78963 18.1371 12.0675C19.3059 14.4052 20 17.5647 20 21H18C18 17.8079 17.3509 14.9674 16.3482 12.9619C15.3156 10.8967 14.074 10 13 10C11.926 10 10.6844 10.8967 9.65179 12.9619Z',
  d7: 'M3 2C3.55229 2 4 2.44772 4 3V14C4 15.6782 4.00213 16.8362 4.11923 17.7072C4.23246 18.5494 4.43644 18.972 4.73223 19.2678C5.02803 19.5636 5.45059 19.7675 6.2928 19.8808C7.16378 19.9979 8.32182 20 10 20H21C21.5523 20 22 20.4477 22 21C22 21.5523 21.5523 22 21 22H9.928C8.33933 22 7.04616 22.0001 6.0263 21.8629C4.96232 21.7199 4.04735 21.4113 3.31802 20.682C2.58869 19.9526 2.28011 19.0377 2.13706 17.9737C1.99994 16.9538 1.99997 15.6607 2 14.072L2 3C2 2.44772 2.44772 2 3 2Z',
  d8: 'M21 21H3V3',
  d9: 'M4 19.5V2.5H2V20.5C2 21.0523 2.44772 21.5 3 21.5H21V19.5H4Z',
  d10: 'M5 4.5H7.00898V6.5H5V4.5ZM7.9982 4.5H10.0072V6.5H7.9982V4.5ZM10.9964 4.5H13.0054V6.5H10.9964V4.5ZM13.9946 4.5H16.0036V6.5H13.9946V4.5ZM16.9928 4.5H19.0018V6.5H16.9928V4.5ZM19.991 4.5H22V6.5H19.991V4.5Z',
  d11: 'M9.65179 12.4619C8.64906 14.4674 8 17.3079 8 20.5H6C6 17.0647 6.69409 13.9052 7.86293 11.5675C9.00187 9.28963 10.7603 7.5 13 7.5C15.2397 7.5 16.9981 9.28963 18.1371 11.5675C19.3059 13.9052 20 17.0647 20 20.5H18C18 17.3079 17.3509 14.4674 16.3482 12.4619C15.3156 10.3967 14.074 9.5 13 9.5C11.926 9.5 10.6844 10.3967 9.65179 12.4619Z',
};

export const IconChartMaximumStrokeRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum StrokeRounded"
    >
      <path 
        d={d.d1}
        i-c="s sr sw"
      />
      <path 
        d={d.d2}
        i-c="s sj sr sw"
      />
      <path 
        d={d.d3}
        i-c="s sj sw"
      />
    </TheIconWrapper>
  );
};

export const IconChartMaximumDuotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum DuotoneRounded"
    >
      <path 
        d={d.d1}
        i-c="s sr sw"
      />
      <path 
        d={d.d2}
        i-c="s sj sr sw"
      />
      <path 
        d={d.d3}
        i-c="o7 s sj sw"
      />
    </TheIconWrapper>
  );
};

export const IconChartMaximumTwotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum TwotoneRounded"
    >
      <path 
        d={d.d1}
        i-c="s sr sw"
      />
      <path 
        d={d.d2}
        i-c="o7 s sj sr sw"
      />
      <path 
        d={d.d3}
        i-c="s sj sw"
      />
    </TheIconWrapper>
  );
};

export const IconChartMaximumSolidRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum SolidRounded"
    >
      <path 
        d={d.d4}
        i-c="c f fr"
      />
      <path 
        d={d.d5}
        i-c="c f fr"
      />
    </TheIconWrapper>
  );
};

export const IconChartMaximumBulkRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum BulkRounded"
    >
      <path 
        d={d.d4}
        i-c="c f fr o7"
      />
      <path 
        d={d.d6}
        i-c="c f fr"
      />
      <path 
        d={d.d7}
        i-c="c f fr"
      />
    </TheIconWrapper>
  );
};

export const IconChartMaximumStrokeSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum StrokeSharp"
    >
      <path 
        d={d.d8}
        i-c="s sj sw"
      />
      <path 
        d={d.d2}
        i-c="s sj ss sw"
      />
      <path 
        d={d.d3}
        i-c="s sj sw"
      />
    </TheIconWrapper>
  );
};

export const IconChartMaximumSolidSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="ChartMaximum SolidSharp"
    >
      <path 
        d={d.d9}
        i-c="c f fr"
      />
      <path 
        d={d.d10}
        i-c="c f fr"
      />
      <path 
        d={d.d11}
        i-c="c f fr"
      />
    </TheIconWrapper>
  );
};

export const iconPackOfChartMaximum: TheIconSelfPack = [
  'ChartMaximum',
  IconChartMaximumStrokeRounded,
  IconChartMaximumDuotoneRounded,
  IconChartMaximumTwotoneRounded,
  IconChartMaximumSolidRounded,
  IconChartMaximumBulkRounded,
  IconChartMaximumStrokeSharp,
  IconChartMaximumSolidSharp,
];