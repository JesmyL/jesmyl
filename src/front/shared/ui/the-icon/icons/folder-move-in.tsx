import React, { FC, JSX } from 'react';
import { TheIconWrapper } from '../icon-wrapper';
import { TheIconProps, TheIconSelfPack } from '../model';

const d: Record<string, string> = {
  d1: 'M7 7H16.75C18.8567 7 19.91 7 20.6667 7.50559C20.9943 7.72447 21.2755 8.00572 21.4944 8.33329C22 9.08996 22 10.1433 22 12.25C22 12.5094 22 12.7592 21.9997 13M12 7L11.3666 5.73313C10.8418 4.68358 10.3622 3.62712 9.19926 3.19101C8.6899 3 8.10802 3 6.94427 3C5.1278 3 4.21956 3 3.53806 3.38032C3.05227 3.65142 2.65142 4.05227 2.38032 4.53806C2 5.21956 2 6.1278 2 7.94427V11C2 15.714 2 18.0711 3.46447 19.5355C4.8215 20.8926 6.94493 20.9921 11 20.9994',
  d2: 'M16.5 16C15.9943 16.4915 14 17.7998 14 18.5M16.5 21C15.9943 20.5085 14 19.2002 14 18.5M14 18.5L22 18.5',
  d3: 'M12 21H13.25C16.7612 21 18.5167 21 19.7779 20.1573C20.3238 19.7926 20.7926 19.3238 21.1573 18.7779C22 17.5167 22 15.7612 22 12.25C22 10.1433 22 9.08996 21.4944 8.33329C21.2755 8.00572 20.9943 7.72447 20.6667 7.50559C19.91 7 18.8567 7 16.75 7H12L11.3666 5.73313C10.8418 4.68358 10.3622 3.62712 9.19926 3.19101C8.6899 3 8.10802 3 6.94427 3C5.1278 3 4.21956 3 3.53806 3.38032C3.05227 3.65142 2.65142 4.05227 2.38032 4.53806C2 5.21956 2 6.1278 2 7.94427V11C2 15.714 2 18.0711 3.46447 19.5355C4.92893 21 7.28595 21 12 21Z',
  d4: 'M21.875 19.6309C22.4273 19.6309 22.875 19.1831 22.875 18.6309C22.875 18.0786 22.4273 17.6309 21.875 17.6309L17.375 17.6309L17.375 17.0366C17.3751 16.861 17.3753 16.6453 17.3532 16.4689L17.3528 16.4655C17.337 16.3391 17.2652 15.763 16.6996 15.4887C16.1327 15.2137 15.6326 15.5157 15.5244 15.581L15.0559 15.9201C14.6801 16.2147 14.1656 16.6204 13.7746 17.0009C13.5795 17.1907 13.3783 17.408 13.2194 17.6389C13.0782 17.8441 12.875 18.1943 12.875 18.625C12.875 19.0557 13.0782 19.4059 13.2194 19.6111C13.3783 19.842 13.5795 20.0593 13.7746 20.2491C14.1656 20.6296 14.6801 21.0352 15.0559 21.3299L15.5244 21.669C15.6326 21.7343 16.1328 22.0363 16.6996 21.7613C17.2652 21.487 17.337 20.9109 17.3528 20.7845L17.3532 20.7811C17.3753 20.6047 17.3751 20.389 17.375 20.2134L17.375 19.6309L21.875 19.6309Z',
  d5: 'M9.3376 2.36396C8.69873 2.12439 7.98705 2.12471 6.95764 2.12516C6.0789 2.12515 5.19705 2.12519 4.6247 2.17564C4.03496 2.22762 3.51888 2.33758 3.04758 2.6006C2.44033 2.93948 1.93928 3.44053 1.6004 4.04778C1.33738 4.51908 1.22742 5.03516 1.17544 5.6249C1.12499 6.19725 1.12499 6.90597 1.125 7.78471V10.9326C1.12499 13.241 1.12498 15.05 1.31475 16.4615C1.50899 17.9062 1.91433 19.0463 2.80914 19.9411C3.70395 20.8359 4.84397 21.2412 6.28871 21.4355C7.70019 21.6252 9.50923 21.6252 11.8176 21.6252C12.2272 21.6252 12.432 21.6252 12.5191 21.541C12.5755 21.4866 12.6056 21.421 12.6102 21.3428C12.6174 21.2218 12.4776 21.059 12.1982 20.7333C12.1132 20.6343 12.0386 20.5409 11.9836 20.4609C11.789 20.1781 11.375 19.5165 11.375 18.6247C11.375 17.7329 11.789 17.0713 11.9836 16.7884C12.2216 16.4424 12.5005 16.1473 12.7286 15.9255C13.1867 15.4797 14.3939 14.5767 14.7588 14.2906C14.9226 14.1907 16.0474 13.5048 17.3542 14.1388C18.1566 14.528 18.5251 15.1705 18.6954 15.6526C18.7829 15.9005 18.8267 16.0245 18.9024 16.0793C18.9781 16.1342 19.0903 16.1368 19.3146 16.1419L21.9664 16.203C22.2327 16.2091 22.3659 16.2122 22.4549 16.1303C22.544 16.0483 22.5518 15.9186 22.5674 15.659C22.625 14.7037 22.625 13.5568 22.625 12.1675C22.625 11.1484 22.625 10.244 22.558 9.58468C22.4887 8.90413 22.3419 8.31392 21.993 7.79181C21.7194 7.38235 21.3679 7.03078 20.9584 6.75719C20.4363 6.40833 19.8461 6.26148 19.1655 6.19225C18.5062 6.12518 17.6845 6.12519 16.6655 6.1252L13.1111 6.1252C12.795 6.1252 12.6146 6.12428 12.4814 6.11051C12.3794 6.10337 12.3098 6.02775 12.2878 5.99083C12.2159 5.87789 11.9588 5.36429 11.8175 5.08156C11.3454 4.12641 10.7331 2.88725 9.3376 2.36396Z',
  d6: 'M21.8457 18.002H14.5649M16.8457 15.004L13.8457 18.002L16.8457 21',
  d7: 'M6.77406 7.00942H21.7374C21.7772 7.00942 21.8116 7.06134 21.8277 7.09033C21.8345 7.10265 21.8374 7.11664 21.8374 7.13073V12.9951M11.8338 20.9876H1.89067C1.83544 20.9876 1.84427 20.9429 1.84427 20.8877L1.84438 3.10223C1.84438 3.04704 1.83555 3.0023 1.89078 3.00229L8.81365 3.00195L11.8699 7.00942',
  d8: 'M17.1644 20.0428L18.4573 21.3357L17.043 22.7499L13.3359 19.0428L17.043 15.3357L18.4573 16.7499L17.1644 18.0428H22.7502V20.0428H17.1644Z',
  d9: 'M2 1.25C1.80109 1.25 1.61032 1.32902 1.46967 1.46967C1.32902 1.61032 1.25 1.80109 1.25 2V20C1.25 20.4142 1.58579 20.75 2 20.75H12.9221L11.2148 19.0428L17.0433 13.2144L20.3717 16.5428H22.75V6C22.75 5.58579 22.4142 5.25 22 5.25H12.375L9.375 1.25001L2 1.25Z',
};

export const IconFolderMoveInStrokeRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn StrokeRounded"
    >
      <path 
        d={d.d1}
        i-c="s sr sw"
      />
      <path 
        d={d.d2}
        i-c="s sj sr sw"
      />
    </TheIconWrapper>
  );
};

export const IconFolderMoveInDuotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn DuotoneRounded"
    >
      <path 
        d={d.d3}
        i-c="f o7"
      />
      <path 
        d={d.d1}
        i-c="s sr sw"
      />
      <path 
        d={d.d2}
        i-c="s sj sr sw"
      />
    </TheIconWrapper>
  );
};

export const IconFolderMoveInTwotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn TwotoneRounded"
    >
      <path 
        d={d.d1}
        i-c="s sr sw"
      />
      <path 
        d={d.d2}
        i-c="o7 s sj sr sw"
      />
    </TheIconWrapper>
  );
};

export const IconFolderMoveInSolidRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn SolidRounded"
    >
      <path 
        d={d.d4}
        i-c="f"
      />
      <path 
        d={d.d5}
        i-c="f"
      />
    </TheIconWrapper>
  );
};

export const IconFolderMoveInBulkRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn BulkRounded"
    >
      <path 
        d={d.d4}
        i-c="f"
      />
      <path 
        d={d.d5}
        i-c="f o7"
      />
    </TheIconWrapper>
  );
};

export const IconFolderMoveInStrokeSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn StrokeSharp"
    >
      <path 
        d={d.d6}
        i-c="s sw"
      />
      <path 
        d={d.d7}
        i-c="s sw"
      />
    </TheIconWrapper>
  );
};

export const IconFolderMoveInSolidSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      the-icon="FolderMoveIn SolidSharp"
    >
      <path 
        d={d.d8}
        i-c="c f fr"
      />
      <path 
        d={d.d9}
        i-c="f"
      />
    </TheIconWrapper>
  );
};

export const iconPackOfFolderMoveIn: TheIconSelfPack = [
  'FolderMoveIn',
  IconFolderMoveInStrokeRounded,
  IconFolderMoveInDuotoneRounded,
  IconFolderMoveInTwotoneRounded,
  IconFolderMoveInSolidRounded,
  IconFolderMoveInBulkRounded,
  IconFolderMoveInStrokeSharp,
  IconFolderMoveInSolidSharp,
];